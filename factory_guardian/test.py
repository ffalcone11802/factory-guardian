...
from factory_guardian.utils.folder import path_joiner
...
def test(args):
    ...
    # Load weights
    model_path = path_joiner("results", "weights", f"{category}.pth")
    ...
    th_path = path_joiner("results", "params", f"{category}.json")
    ...
    if last_batch is not None:
        plot_qualitative_results(*last_batch)
    ...
    # ROC curves
    plot_partial_roc(img_true, img_scores, img_level_th, category)
    plot_partial_roc(px_true, px_scores, px_level_th, category, max_fpr=True, pixel_level=True)

    # Inference time and FPS
    dummy_input = torch.randn(1, 3, img_size, img_size).to(device)
    ms, fps = inference_time(model, dummy_input, warmup_epochs=20, N=1000)
    print(f"Average inference time: {ms:.4f} ms, FPS: {fps:.4f} fps")


def inference_time(model, x, warmup_epochs=20, N=1000):
